...
RPROMPT='$(_vi_status)%{$(echotc UP 1)%}$(_git_time_since_commit) $(git_prompt_status) $(_disk_usage) ${_return_status}%{$(echotc DO 1)%}'
...
function _disk_usage() {
  ...
  if [ "$disk_usage" -le "$warn_level" ]; then 
    echo "%{$fg_bold[green]%}${disk_usage}%\%%%{$reset_color%}"
  else 
    echo "%{$fg_bold[yellow]%}${disk_usage}%\%%%{$reset_color%}"
  fi
}
